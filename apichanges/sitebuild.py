...
from .icons import get_icon_style, get_icon
from .model import ServiceModel, ReleaseDelta
from .record import Commit, ServiceChange # noqa
from .repo import TagWalker, CommitProcessor
...
class TemplateAPI:
    ...
    def _get_service_model(self, service_change):
        ...
        if service_change.model_file == GIT_EMPTY_FILE:
            return
        data = json.loads(self.repo[
            service_change.model_file].read_raw().decode('utf8'))
        m = ServiceModel(data)
...
class Site:
    ...
    def build(self, output: Path, destination: Optional[str] = None):
        ...
        self.build_index_pages(
            self.commits[:bisect_create_age(self.commits, 60)])
        self.build_feed(
            self.commits[:bisect_create_age(
                self.commits, days=60)])
        if not new_commits:
            log.info('no changes')
...
#                :bisect_create_age(self.commits, self.default_commit_days)])
            ...
            self.build_service_pages(
                self.commits, set(group_by_service(new_commits)))
            self.build_search_index(
                self.commits[:bisect_create_age(
                    self.commits, days=365 + 60)])
    ...
    def render_page(self, path,
                    template: Optional[str] = None, force: bool = False,
                    **kw):
        if template:
    ...
    def build_feed(self, commits: List[Commit]):
        ...
        feed.image(url='https://a0.awsstatic.com/main/images/logos/aws_logo_smile_179x109.png') # noqa
        for c in commits:
            for s in c.service_changes:
    ...
    def build_commit_pages(self, commits: List[Commit]):
        for c in commits:
            for svc_change in c:
                self.render_page(
                    'archive/changes/{}-{}.html'.format(
                        c.id[:6], svc_change.name),
                    'service-commit.j2',
    ...
    def build_service_pages(self, commits: List[Commit], services=None):
        ...
        self.render_page(
            ...
            'service-map.j2',
            services=sorted(
                groups, key=lambda s: groups[s][0].created, reverse=True),
            changes=groups,
            force=True)
...
#        log.info('build main pages %d' % len(pager['pages']))
        for idx, batch in [(0, commits)]:
            p = '%s.html' % (
                idx and 'archive/index/%d' % idx or 'index')
            log.info(
                'main page: %s commits: %d changes: %d start: %s  period: %s',
                str(p), len(batch),
                sum([len(s) for s in itertools.chain(
                    *[c for c in batch])]),
                batch[-1].created.strftime('%Y-%m-%d'),
                (batch[0].created - batch[-1].created).days)
            self.render_page(
                p, 'index.j2', releases=batch, pager=pager, force=True)

    def load(self, repo_path: str, cache_path: str,
    ...
    def _load(self, repo_path: str,
        ...
        delta = CommitProcessor(
            repo, change_dir='.changes',
            model_prefix='apis/', model_suffix='normal.json')
        releases = []
